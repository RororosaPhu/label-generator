...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        # 如果 secure_filename 移除了所有字符，使用原始文件名
        if not safe_filename or safe_filename == '.xlsx' or safe_filename == '.xls':
            safe_filename = original_filename
        excel_path = os.path.join(UPLOAD_FOLDER, f"{unique_id}_{safe_filename}")
        ...
        try:
            count = generate_pdf(excel_path, pdf_path, original_filename)
